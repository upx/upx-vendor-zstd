...
rm -r lib/deprecated lib/dictBuilder lib/dll lib/legacy
rm lib/.gitignore lib/BUCK lib/Makefile lib/*.in lib/*.md lib/*.mk lib/*.modulemap
# remove unused assembly and multithreading files
rm lib/*/*.S lib/common/pool.[ch] lib/common/threading.[ch] lib/compress/zstdmt_*.[ch]
# 5) cleanup files (whitespace)
find . -type d -name '.git' -prune -o -type f -print0 | xargs -0r sed -i -e 's/[ \t]*$//'
for f in lib/*/*.[ch]; do expand -i -t 4 $f > $f.tmp; mv $f.tmp $f; done
